...
describe("LavaJs", function() {
  ...
  describe("create", function() {
    it("should create a new Chart from a JSON payload.", function() {
      const chart = lava.create(getPieChartJson());
    ...
    it("should have a Chart and DataTable after lava.run()", function() {
      const chart = lava.create(getPieChartJson());
      const lavaRunSpy = sinon.spy(chart, "run");
      ...
      return lava.run().then(function() {
        ...
        expect(chart.data).to.shallowDeepEqual(
  ...
  describe("query", function() {
    const testUrl = "http://not.a.real.url.com";

    it("should create a new DataQuery from string URL.", function() {
      const query = lava.query(testUrl);
  ...
  describe("loadData", function() {
    let chart, lavaRunSpy, chartSetDataSpy, chartDrawSpy, chartApplyFormatsSpy;

    const data = getDataTableJson();
    const formattedData = {
      data: data,
      formats: [getNumberFormat(1)]
    };
        ...
        it("should update the DataTable.", function() {
          return lava.run().then(function() {
            ...
            lava.loadData("MyCoolChart", data, function(newData) {
              ...
              expect(newData).to.shallowDeepEqual(
                new google.visualization.DataTable()
              );
            });
          });
      ...
      describe("and with formats", function() {
        it("should apply the formats while updating the DataTable.", function() {
          return lava.run().then(function() {
            ...
            return lava.loadData("MyCoolChart", formattedData, function(
              newData
            ) {
    ...
    describe("with a Function", function() {
      const callbackData = function(data) {
  ...
  describe("loadOptions", function() {
    let chart, lavaRunSpy, chartDrawSpy;
